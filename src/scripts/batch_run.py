...
for epoch in epochs:
    for dup in dups:
        ...
        command = " \n " .join([
            f"cd /workspaces/workspaces/reps/implicit_chain_of_thought",
            f"conda activate icot",
            f"set -xg CUDA_VISIBLE_DEVICES {exp_id%4}",
            f"{exp_command_0_dict[args.exp]}",
            f"mkdir -p generation_logs/{exp_str}/gpt2",
            f"mkdir -p train_models/{exp_str}/gpt2",
            f"date >generation_logs/{exp_str}/gpt2/before.txt",
            f"python src/train_teacher.py --train_path data/{data_str}/train.txt --val_path data/{data_str}/valid.txt --epochs {epoch} --lr 5e-5 --batch_size 32 --base_model /mnt/cephfs/zhangyi.justice/workspaces/downloaded_models/gpt2 --save_model train_models/{exp_str}/gpt2/teacher 2>&1 >generation_logs/{exp_str}/gpt2/teacher.txt",
            f"python src/train_mind_reading_student.py --train_path data/{data_str}/train.txt --val_path data/{data_str}/valid.txt --epochs {epoch} --lr 5e-5 --batch_size 32 --base_model /mnt/cephfs/zhangyi.justice/workspaces/downloaded_models/gpt2 --teacher train_models/{exp_str}/gpt2/teacher/checkpoint_{epoch-1} --save_model train_models/{exp_str}/gpt2/student_initial --delta dynamic 2>&1 >generation_logs/{exp_str}/gpt2/student.txt",
            f"python src/train_thought_emulator.py --train_path data/{data_str}/train.txt --val_path data/{data_str}/valid.txt --epochs {epoch} --lr 5e-5 --batch_size 32 --base_model /mnt/cephfs/zhangyi.justice/workspaces/downloaded_models/gpt2 --teacher train_models/{exp_str}/gpt2/teacher/checkpoint_{epoch-1} --save_model train_models/{exp_str}/gpt2/emulator_initial --delta dynamic --subset {exp_command_1_dict[args.exp]} --mixture_size 1 2>&1 >generation_logs/{exp_str}/gpt2/emulator.txt",
            ...
            f"date >generation_logs/{exp_str}/gpt2/after.txt",
