    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if mode == "forward_emulator":
                ...
                f_h_c = mlps[i](
                    torch.cat((z, mixture_embedding), dim=-1)
                )  # bsz, hidden_size
                ...
                if rnn is not None:
                    ...
                    if key_proj is not None:
                        ...
                        output, rnn_state = rnn(
                            (next_input + context).unsqueeze(0), rnn_state
                        )
                        ...
                        if past_keys is not None:
                            ...
                            past_keys = torch.cat(
                                (past_keys, current_key.unsqueeze(1)), dim=1
                            )
                        else:
                ...
                if positions_to_substitute.eq(positions_to_substitute[0]).all():
                    ...
                    for batch_id in range(batch_size):
                        hidden_states[
                            batch_id, positions_to_substitute[batch_id]
